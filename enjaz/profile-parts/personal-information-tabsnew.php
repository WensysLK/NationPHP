...
if (isset($_GET['client_id'])) {
    ...
    $sqlnewQuery = "SELECT *
                     FROM `enjaz_details` 
                     WHERE `softdeletestatus` = 1 AND `CleintId` = ?";

    $stmt6New = $conn->prepare($sqlnewQuery); // Use prepared statements to avoid SQL injection
    $stmt6New->bind_param("i", $client_id); // Bind the client ID as an integer
    $stmt6New->execute();
    $result6New = $stmt6New->get_result(); // Get the result set
    $applicantNew = $result6New->fetch_assoc();
    ...
    WHERE c.`applicant_id` = ? AND c.softdeletestatus = 1";
     ...
     (isset($contact['applicant_add2']) ? $contact['applicant_add2'] : ''); ?></td>
                ...
                <div class="col-md-12">
                    ...
                    <form action="functions/medical_payment_insert.php" method="POST"
                          enctype="multipart/form-data">
                        <input type="hidden" name="appId" value="<?php echo $client_id?>" id="appId">
<!--                        <input type="hidden" name="contractId" value="--><?php //echo $contractId?><!--" id="contractId">-->
                        <input type="hidden" name="medcialId" value="<?php echo $applicantNew['EnjazId']?>" id="medicalId">
                        <div class="row">

...
if (isset($_GET['query'])) {
    ...
    $stmt = $conn->prepare("SELECT localagentId, Local_Agent_Title, Local_Agent_Fname, Local_Agent_Lname, Local_Agent_Nic, Local_Agent_Phone, localagnetProfile  FROM local_agent_details WHERE Local_Agent_Fname LIKE ? OR Local_Agent_Nic LIKE ? OR Local_Agent_Phone LIKE ?");
    $searchTerm = '%' . $query . '%';
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            $subAgents[] = [
                ...
                'image' => $row['localagnetProfile']
                // Correct field name from SQL query
            ];
        }
    }

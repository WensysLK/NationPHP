...
//var_dump($smtpmedicalupdate->execute());die();
        ...
        if ($smtpmedicalupdate->execute()) {
            ...
            $sqlmedicalupdates = "UPDATE `complains_follow_up` SET 
            ...
            $smtpmedicalupdate = $conn->prepare($sqlmedicalupdates);
            $smtpmedicalupdate->bind_param("sssssi",
                $followup_type,
                $message,
                $status,
                $date,
                $status_delete,// medicalresult (string)
                $follupid          // medicalId (integer)
            );
            $smtpmedicalupdate->execute();
...
//            $stmt->execute();
            ...
            header("Location: ../help_desk_process_parts/followup.php?complain_id=$complainant_id");
        } else {
